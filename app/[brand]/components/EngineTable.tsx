...
const EngineTable = <T extends Record<string, unknown>>({
  ...
  // Determine if cell content should be bold
  const shouldBeBold = (columnKey: string) => {
    switch (tableType) {
      case "models":
        return columnKey === "model_name";
  ...
  const tableContainerClass = `hidden lg:block bg-card rounded-2xl shadow-lg mb-5 overflow-x-auto ${getWidthClass()} mx-auto`;
                  ...
                  return (
                    <TableCell
                      ...
                      className={`px-4 py-2 text-foreground ${
                        ...
                        col.key === "action"
                          ? "text-right whitespace-nowrap" // Buttons don't wrap
